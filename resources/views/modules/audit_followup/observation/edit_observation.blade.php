                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-control rounded-0 select-select2" id="observation_type"
                                    ...
                                    <option value="SFI"
                                    {{ $data['observation_type'] == "SFI" ? 'selected' : '' }}
                                    >SFI</option>
                                    <option value="NON-SFI"
                                    {{ $data['observation_type'] == "NON-SFI" ? 'selected' : '' }}
                                    >NON-SFI</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="" class="col-form-label">Observation title english
                                        :</label>
                                        <input type="text" name="observation_en" value="{{ $data['observation_en'] }}" class="form-control rounded-0" placeholder="" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="" class="col-form-label">Observation title bangla
                                        :</label>
                                        <input type="text"  name="observation_bn" value="{{ $data['observation_bn'] }}" class="form-control rounded-0" placeholder="" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="" class="col-form-label">Observation details
                                        :</label>
                                        <textarea name="observation_details" id="editor">{!! $data['observation_details'] !!}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="" class="col-form-label">Amount
                                        :</label>
                                        <input type="text" name="amount" value="{{ $data['amount'] }}" class="form-control rounded-0" placeholder="" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="" class="col-form-label">Initiation date
                                        :</label>
                                        <input type="date"  name="initiation_date" value="{{ $data['initiation_date'] }}" class="form-control rounded-0" placeholder="" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="thumbs" style="width: unset;margin: unset;text-align: unset;" data-gallery="one">
                                    @foreach ($data['attachments'] as $attachment)
                                        <a target="_blank" href="{{$attachment['file_url']}}" style="background-image:url({{ in_array($attachment['file_type'], $type) ? $attachment['file_url'] : asset('assets/images/doc.png') }})" title="{{$attachment['file_name']}}"></a>
                                        <button onclick="deleteImage('{{ route('audit.followup.observation.image.delete',$attachment['id']) }}')" type="button" class="button"><i style="color: crimson;" class="fas fa-trash"></i></button>
                                    @endforeach
                                </div>
                            </div>
                        </div>
...
$('#submit_form').click(function () {
    ...
    $.ajax({
        url: "{{route('audit.followup.observation.update')}}", 
        type: "POST",             
        ...
        if (result.isConfirmed) {
            ajaxCallAsyncCallbackAPI(url, {}, 'GET', function (response) {
                if (response.status === 'error') {
                    ...
                    loadPage('{{ route('audit.followup.observation.edit', $data['id']) }}');
                }
            });
