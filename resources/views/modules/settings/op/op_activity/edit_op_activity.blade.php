<x-modal id="op_activity_modal" title="Create Operation Activity"
         url="{{route('settings.operational-plan.activity.update', ['activity' => $activity['id']])}}" method="put">
    <form id="op_activity_form">
            ...
            <div class="col-9">
                <select name="duration_id" id="duration_id" class="form-control select-select2">
                    ...
                    @forelse($plan_durations as $plan_duration)
                        <option
                            value="{{$plan_duration['id']}}" {{$plan_duration['id'] == $activity['duration_id'] ? "selected" : ''}}>{{$plan_duration['start_year']}}
            ...
            <div class="col-9">
                <select name="outcome_id" id="outcome_id" class="form-control select-select2">
                    ...
                    @forelse($plan_outcomes as $plan_outcome)
                        <option
                            value="{{$plan_outcome['id']}}" {{$plan_outcome['id'] == $activity['outcome_id'] ? "selected" : ''}}>{{$plan_outcome['outcome_no']}}</option>
                    @empty
                    @endforelse
                </select>
            </div>
            ...
            <div class="col-9">
                <select name="output_id" id="output_id" class="form-control select-select2">
                    ...
                    @forelse($plan_outputs as $plan_output)
                        <option
                            value="{{$plan_output['id']}}" {{$plan_output['id'] == $activity['output_id'] ? "selected" : ''}}>{{$plan_output['output_no']}}</option>
                    @empty
                    @endforelse
                </select>
            </div>
            ...
            <div class="col-9">
                <select name="activity_parent_id" id="activity_parent_id" class="form-control select-select2">
                    ...
                    @forelse($op_activities as $op_activity)
                        <option
                            value="{{$op_activity['id']}}" {{$op_activity['id'] == $activity['activity_parent_id'] ? "selected" : ''}}>{{$op_activity['activity_no']}}</option>
                    @empty
                    @endforelse
                </select>
            </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-9">
                <input placeholder="Activity No" class="form-control" type="text" value="{{ $activity['activity_no'] }}"
        ...
        <div class="form-group row">
            ...
            <div class="col-9">
                <input placeholder="Title English" class="form-control" type="text" value="{{ $activity['title_en'] }}"
        ...
        <div class="form-group row">
            ...
            <div class="col-9">
                <input placeholder="Title Bangla" class="form-control" type="text" value="{{ $activity['title_bn'] }}"
