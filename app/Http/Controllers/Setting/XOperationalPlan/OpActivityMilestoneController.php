...
class OpActivityMilestoneController extends Controller
{
    ...
    {
        return view('modules.settings.op.op_activity_milestone.activity_milestone_lists');
    }

    public function getActivityLists(Request $request)
    {
        $op_activities = $this->initHttpWithToken()->post(config('amms_bee_routes.settings.op_activity_milestone_lists'), [
            'all' => 1
        ])->json();
        if ($op_activities['status'] == 'success') {
            $op_activities = $op_activities['data'];
            return view('modules.settings.op.op_activity_milestone.partials.get_activity_milestone_lists', compact('op_activities'));
        } else {
