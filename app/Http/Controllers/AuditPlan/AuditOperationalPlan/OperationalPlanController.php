...
class OperationalPlanController extends Controller
{
    public function index()
    {
        ...
        return view(
            'modules.audit_plan.operational.operational_plan.operational_plan_lists',
            compact('fiscal_years')
        );
    }
    ...
    public function showOperationalPlanLists(Request $request)
    {
        ...
        $ops = $this->initHttpWithToken()
            ->post(
                config('amms_bee_routes.audit_operational_plan.load-operational-plan-lists'),
        ...
        if ($activityMilestones['status'] = 'success') {
            $ops = $ops['data'];
            return view(
                'modules.audit_plan.operational.operational_plan.partials.load_operational_plan_lists',
                compact('ops')
            );
        } else {
